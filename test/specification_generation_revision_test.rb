...
class SpecificationGenerationRevisionTest < Minitest::Test
  ...
  def start_with_revision(provider: nil, gh_seed: [], branch_exists: true)
    ensure_remote_checkout
    ...
    @platform = FakePlatform.new(
      claim_payload: spec_creation_payload_for(issue_key: ISSUE, existing_pull_request_url: PR_URL)
    ).start
    @config = build_config(provider: provider)
  end
